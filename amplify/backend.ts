...
// This explicit, branch-scoped switch is used only for the removal deployment.
// Keep the identity table in both stages so existing account metadata is preserved.
const resetTarget = process.env.FIN_VISION_AUTH_RESET;
const resetAuth = Boolean(resetTarget);
if (resetAuth && (!process.env.AWS_APP_ID || process.env.AWS_BRANCH !== 'dev' ||
    resetTarget !== `${process.env.AWS_APP_ID}/dev`)) {
  throw new Error('FIN_VISION_AUTH_RESET must match <AWS_APP_ID>/dev and may only run on the dev branch.');
}
const deployment = resetAuth
  ? {mode:'reset' as const, backend:defineBackend({})}
  : {mode:'auth' as const, backend:defineBackend({ auth })};
...
if (deployment.mode === 'auth') {
  ...
  backend.addOutput({custom:{auth_reset_in_progress:true}});
}
