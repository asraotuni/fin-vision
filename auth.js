...
function lockPlanner(message = 'Sign in with Google to continue.'){
  sessionGeneration += 1;
  signedIn = false;
  tokenClient = null;
...
async function synchronize(){
  ...
  if(generation !== sessionGeneration) return;
  signedIn = true;
  byId('accountName').textContent = payload.name || [payload.given_name, payload.family_name].filter(Boolean).join(' ') || 'Not provided by Google';
...
byId('shareGoogleProfileBtn').addEventListener('click', () => {
  if(!signedIn || !tokenClient) return;
...
async function initialize(){
  try {
    ...
    if(!outputs.auth?.oauth) throw new Error('Missing Google provider');
    ...
    lockPlanner('Google sign-in is not available yet. Please try again once setup is complete.');
    byId('googleSignInBtn').disabled = true;
  }
}
